...
void printUsage(std::string& error) {
  const std::string usage =
      ...
      "  add <name>[@ver]    Install a registry package into ./libs\n"
...
"  --branch-opt        CFG simplification and jump threading\n"
       "  --jump-threading    Thread jumps across constant branches\n"
       "  --sroa              Scalar replacement of aggregates\n"
       "  --mem2reg           Promote allocas to SSA registers\n"
       "  --sccp              Sparse conditional constant propagation\n"
       "  --indvars           Simplify induction variables in loops\n"
       "  --unroll-and-jam    Unroll and jam inner loops\n"
       "  --sink              Sink common instructions out of hot paths\n"
       "  --arg-promote       Promote by-pointer arguments\n"
...
[[nodiscard]] bool parseProjectCommand(std::string_view argument, ProjectCommand& command) {
  ...
  if (argument == "add" || argument == "install") {
...
bool parseCommandLine(int argc, char** argv, CompilerOptions& options, std::string& error) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (options.projectCommand == ProjectCommand::None && parseProjectCommand(argument, command)) {
      ...
      if (command == ProjectCommand::Add && index + 1 < argc && argv[index + 1][0] != '-') {
        ++index;
    ...
    if (options.projectCommand == ProjectCommand::Add && options.packageSpec.empty()) {
      options.packageSpec = std::string(argument);
      continue;
    }
