...
IRType IRType::voidType() {
  return IRType(Kind::Void);
}
IRType IRType::boolType() {
  return IRType(Kind::Bool);
}
IRType IRType::i8() {
  return IRType(Kind::I8);
}
IRType IRType::i16() {
  return IRType(Kind::I16);
}
IRType IRType::i32() {
  return IRType(Kind::I32);
}
IRType IRType::i64() {
  return IRType(Kind::I64);
}
IRType IRType::u8() {
  return IRType(Kind::U8);
}
IRType IRType::u16() {
  return IRType(Kind::U16);
}
IRType IRType::u32() {
  return IRType(Kind::U32);
}
IRType IRType::u64() {
  return IRType(Kind::U64);
}
IRType IRType::f32() {
  return IRType(Kind::F32);
}
IRType IRType::f64() {
  return IRType(Kind::F64);
}
...
IRType IRType::label() {
  return IRType(Kind::Label);
}
IRType IRType::stringType() {
  return IRType(Kind::String);
}
IRType::Kind IRType::kind() const {
  return kind_;
}
const IRType* IRType::pointee() const {
  return pointee_.get();
}
const std::vector<IRType>& IRType::elements() const {
  return elements_;
}
const std::vector<IRType>& IRType::parameters() const {
  return parameters_;
}
const std::string& IRType::name() const {
  return name_;
}
std::int64_t IRType::length() const {
  return length_;
}
...
std::string IRType::display() const {
  ...
  case Kind::F64:
    return "f64";
  case Kind::Ptr:
    return "Ptr[" + pointee_->display() + "]";
  case Kind::Array:
    return "[" + std::to_string(length_) + " x " + elements_[0].display() + "]";
  case Kind::Function: {
    std::string result = "(";
...
const IRType& Value::type() const {
  return type_;
}
std::string Value::reference() const {
  return display();
}
...
ValueKind ConstantInt::valueKind() const {
  return ValueKind::ConstantInt;
}
std::string ConstantInt::display() const {
  return std::to_string(value_);
}

ConstantFloat::ConstantFloat(double value, IRType type) : Value(std::move(type)), value_(value) {
}
double ConstantFloat::value() const {
  return value_;
}
ValueKind ConstantFloat::valueKind() const {
  return ValueKind::ConstantFloat;
}
...
ConstantString::ConstantString(std::string value)
    : Value(IRType::stringType()), value_(std::move(value)) {
}
...
const std::string& ConstantString::globalName() const {
  return globalName_;
}
ValueKind ConstantString::valueKind() const {
  return ValueKind::ConstantString;
}
std::string ConstantString::display() const {
  return "\"" + value_ + "\"";
}
...
ValueKind Argument::valueKind() const {
...
FunctionRef::FunctionRef(std::string name, IRType type)
    : Value(std::move(type)), name_(std::move(name)) {
}
const std::string& FunctionRef::name() const {
  return name_;
}
ValueKind FunctionRef::valueKind() const {
  return ValueKind::FunctionRef;
}
std::string FunctionRef::display() const {
  return "@" + name_;
}
...
const std::string& Operation::opcode() const {
  return opcode_;
}
const std::string& Operation::resultName() const {
  return resultName_;
}
const std::vector<ValuePtr>& Operation::operands() const {
  return operands_;
}
const std::unordered_map<std::string, std::string>& Operation::attributes() const {
  return attributes_;
}
void Operation::setOperands(std::vector<ValuePtr> operands) {
  operands_ = std::move(operands);
}
ValueKind Operation::valueKind() const {
...
const std::string& BasicBlock::label() const {
  return label_;
}
const std::vector<std::shared_ptr<Operation>>& BasicBlock::operations() const {
  return operations_;
}
bool BasicBlock::isTerminated() const {
  return terminated_;
}
void BasicBlock::append(std::shared_ptr<Operation> operation) {
  operations_.push_back(std::move(operation));
}
void BasicBlock::setOperations(std::vector<std::shared_ptr<Operation>> operations) {
  operations_ = std::move(operations);
}
void BasicBlock::setTerminated() {
  terminated_ = true;
}
std::string BasicBlock::display() const {
  std::string text = label_ + ":\n";
  for (const auto& operation : operations_)
    text += "  " + operation->display() + "\n";
  return text;
}
...
IRFunction::IRFunction(std::string name, std::vector<IRType> parameters, IRType result)
  ...
  for (std::size_t index = 0; index < parameters_.size(); ++index) {
    arguments_.push_back(
        std::make_shared<Argument>("arg" + std::to_string(index), parameters_[index]));
  }
}
const std::string& IRFunction::name() const {
  return name_;
}
const std::vector<IRType>& IRFunction::parameters() const {
  return parameters_;
}
const IRType& IRFunction::resultType() const {
  return result_;
}
const std::vector<std::shared_ptr<Argument>>& IRFunction::arguments() const {
  return arguments_;
}
const std::vector<std::unique_ptr<BasicBlock>>& IRFunction::blocks() const {
  return blocks_;
}
...
std::string IRFunction::nextValueName() {
  return std::to_string(nextValue_++);
}
void IRFunction::setAsync(bool value) {
  async_ = value;
}
void IRFunction::setGenerator(bool value) {
  generator_ = value;
}
void IRFunction::setExternal(bool value) {
  external_ = value;
}
bool IRFunction::isExternal() const {
  return external_;
}
...
std::string IRFunction::display() const {
  std::string text = std::string(external_ ? "extern " : "") + std::string(async_ ? "async " : "") +
...
const IRType& TypeDef::type() const {
  return type_;
}
const std::vector<std::string>& TypeDef::attributes() const {
  return attributes_;
}
...
const std::string& GlobalConstant::name() const {
  return name_;
}
const IRType& GlobalConstant::type() const {
  return type_;
}
const std::string& GlobalConstant::value() const {
  return value_;
}
...
const std::string& IRModule::name() const {
  return name_;
}
...
const std::vector<std::unique_ptr<GlobalConstant>>& IRModule::globals() const {
  return globals_;
}
const std::vector<std::unique_ptr<IRFunction>>& IRModule::functions() const {
  return functions_;
}
...
IRBuilder::IRBuilder(IRFunction& function)
    : function_(&function), block_(&function.addBlock("entry")) {
}
void IRBuilder::setInsertBlock(BasicBlock& block) {
  block_ = &block;
}
BasicBlock& IRBuilder::currentBlock() const {
  return *block_;
}
std::shared_ptr<Operation>
IRBuilder::operation(std::string opcode,
                     IRType resultType,
                     std::vector<ValuePtr> operands,
                     std::unordered_map<std::string, std::string> attributes) {
  const std::string resultName = resultType.isVoid() ? "" : function_->nextValueName();
  auto result = std::make_shared<Operation>(std::move(opcode),
                                            std::move(resultType),
                                            resultName,
                                            std::move(operands),
                                            std::move(attributes));
...
std::shared_ptr<Operation> IRBuilder::compare(std::string predicate, ValuePtr lhs, ValuePtr rhs) {
  return operation(
      "cmp." + std::move(predicate), IRType::boolType(), {std::move(lhs), std::move(rhs)});
}
...
std::shared_ptr<Operation> IRBuilder::getElement(ValuePtr aggregate, ValuePtr index, IRType type) {
...
std::shared_ptr<Operation> IRBuilder::extract(ValuePtr aggregate, std::size_t index, IRType type) {
  return operation(
      "extract", std::move(type), {std::move(aggregate)}, {{"index", std::to_string(index)}});
}
std::shared_ptr<Operation>
IRBuilder::insert(ValuePtr aggregate, ValuePtr value, std::size_t index, IRType type) {
  return operation("insert",
                   std::move(type),
                   {std::move(aggregate), std::move(value)},
                   {{"index", std::to_string(index)}});
}
std::shared_ptr<Operation>
IRBuilder::call(ValuePtr callee, std::vector<ValuePtr> arguments, IRType resultType) {
  std::vector<ValuePtr> operands;
...
std::shared_ptr<Operation>
IRBuilder::select(ValuePtr condition, ValuePtr ifTrue, ValuePtr ifFalse, IRType type) {
  return operation(
      "select", std::move(type), {std::move(condition), std::move(ifTrue), std::move(ifFalse)});
}
...
std::shared_ptr<Operation>
IRBuilder::conditionalBranch(ValuePtr condition, BasicBlock& ifTrue, BasicBlock& ifFalse) {
  auto result = operation("cond_branch",
                          IRType::voidType(),
                          {std::move(condition)},
...
std::shared_ptr<Operation> IRBuilder::coroBegin(IRType element, bool generator) {
  return operation("coro.begin",
                   IRType::ptr(std::move(element)),
                   {},
...
std::shared_ptr<Operation>
IRBuilder::asyncCreate(ValuePtr function, std::vector<ValuePtr> arguments, IRType type) {
  std::vector<ValuePtr> operands;
...
std::shared_ptr<Operation>
IRBuilder::invoke(ValuePtr callee, std::vector<ValuePtr> arguments, IRType resultType) {
  std::vector<ValuePtr> operands;
  operands.push_back(std::move(callee));
  for (auto& argument : arguments)
    operands.push_back(std::move(argument));
