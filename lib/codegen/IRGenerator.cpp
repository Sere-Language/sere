...
llvm::Value* IRGenerator::emitMethodCall(llvm::IRBuilder<>& builder, const CallExpr& expr) {
  ...
  if (methodDef != nullptr && methodDef->decoratedType() != nullptr) {
    llvm::Value* current = packCallable(builder, found->second, thisPtr);
    const Type* rawType = methodDef->resolvedType();
    std::vector<llvm::Type*> valueTypes;
    for (std::size_t index = 1; rawType != nullptr && index < rawType->paramTypes().size(); ++index)
      valueTypes.push_back(lower(rawType->paramTypes()[index]));
    llvm::FunctionType* valueType = llvm::FunctionType::get(
        rawType == nullptr || rawType->returnType() == nullptr
            ? llvm::Type::getVoidTy(*context_)
            : lower(rawType->returnType()),
        valueTypes,
        false);
    for (int index = static_cast<int>(methodDef->decoratorExprs().size()) - 1; index >= 0;
         --index) {
      Expr& decorator = *methodDef->decoratorExprs()[static_cast<std::size_t>(index)];
      if (isReservedDecoratorExpr(decorator))
        continue;
      llvm::Value* wrapper = emitExpr(builder, decorator);
      const Type* wrapperType = decorator.resolvedType();
      if (wrapper == nullptr || wrapperType == nullptr || wrapperType->kind() != TypeKind::Function)
        continue;
      llvm::FunctionType* decoratorType = llvmFunctionTypeFrom(wrapperType);
      current = emitIndirectCallable(builder, wrapper, decoratorType, {current});
    }
    std::vector<llvm::Value*> valueArgs(args.begin() + 1, args.end());
    return emitIndirectCallable(builder, current, valueType, valueArgs);
  }
